...
DATA=${NAME[1]}## DATA="config"
# get config name
CFG_NAME=${NAME[2]%".yml"} ##提取得到 CFG_NAME=“evaluate”
...
#  	nohup sh -c 'python -u code/faith/distant_supervision/distant_supervision.py ${FUNCTION} ${CONFIG}' > $OUT 2>&1 &
    nohup sh -c 'python -u faith/distant_supervision/distant_supervision.py ${FUNCTION} ${CONFIG}' > $OUT 2>&1 &
else
	# run with sbatch
	sbatch <<EOT
#!/bin/bash
