...
class Seq2SeqIQUESModel(torch.nn.Module):
    def __init__(self, config):
        ...
        elif config["iques_architecture"] == "Pegasus":
            self.model = transformers.PegasusForConditionalGeneration.from_pretrained("google/pegasus-large")
            self.tokenizer = transformers.PegasusTokenizer.from_pretrained("google/pegasus-large")
        else:
            raise Exception(
                "Unknown architecture for IQUES module specified in config: currently, only T5-base (=T5) and BART-base (=BART) are supported."
                "Unknown architecture for IQUES module specified in config: currently, T5, BART and Pegasus are supported."
            )
