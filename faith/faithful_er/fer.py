...
class FER(FaithfulEvidenceRetrieval):
    ...
    def inference_on_instance(self, instance, sources=["kb", "text", "table", "info"]):
        ...
        tsf = instance["structured_temporal_form"]#检索结构化时间形式（TSF）
        # tsf is a dictionary
        if isinstance(tsf, dict):#确认TSF是否为词典
        ...
        initial_hit, initial_answering_evidences = answer_presence(initial_evidences, instance["answers"])#回答存在评估
        ...
        if self.faith_or_unfaith == "faith":#修剪（如果忠实）
        ...
        self.logger.debug(f"Time taken (ER, EP): {time.time() - start} seconds")#结束计时器和日志执行时间
        # store the evidences with faithful tag
        top_evidences = self.evs.get_top_evidences(query, instance["candidate_evidences"], self.max_evidence)#存储top证据
