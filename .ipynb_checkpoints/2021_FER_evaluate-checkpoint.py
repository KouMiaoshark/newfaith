...
def load_rel_map(rel_json_path, key="Relation"):
    ...
    返回: {Id: relation_str}
    """
    data = json.load(open(rel_json_path, "r", encoding="utf-8"))
    rel_map = {}
    for item in data:
        # 允许 item["Id"] 是 str/int
        qid = int(item["Id"])
        rel_map[qid] = str(item[key]).strip()
    return rel_map


def filter_instances(base_instances, rel_map, max_id=3000, skip_ids=None):
    """
    只保留：
      1) 1<=Id<=max_id（这里用 Id<=max_id）
    ...
    if missing_stf:
        # 这里直接报错更好：说明 BASE_TSF 不是 TQU 输出或被破坏
        raise ValueError(f"以下 Id 缺 structured_temporal_form（BASE_TSF 可能不是TQU输出）：{missing_stf[:20]} ... 共{len(missing_stf)}条")
...
def inject_relation(instances, rel_map):
    ...
    for ins in instances:
        qid = int(ins["Id"])
        # filter_instances 已保证 structured_temporal_form 存在且 qid in rel_map
        ins["structured_temporal_form"]["relation"] = rel_map[qid]
        changed += 1
    return changed


def run_one(config_path, base_tsf_data_path, rel_json_path, rel_key, out_path, sources,
            max_id=3000, skip_ids=None):
    if skip_ids is None:
        skip_ids = set()
    ...
    # 再加载 base TSF 数据
    base_data = json.load(open(base_tsf_data_path, "r", encoding="utf-8"))
    ...
    # 过滤：只跑 Id<=max_id；跳过 skip_ids；且必须在 rel_map 中有 relation
    data = filter_instances(data, rel_map, max_id=max_id, skip_ids=skip_ids)

    # 打印一下实际将要跑的范围与数量（注意：2525会被跳过）
    ids = [int(x["Id"]) for x in data]
    if ids:
        print(f"[filter] 将处理 {len(data)} 条：Id {min(ids)} ~ {max(ids)}（max_id={max_id}，skip={sorted(list(skip_ids))}）")
        if 2525 in ids:
            raise RuntimeError("逻辑错误：2525 不应出现在处理列表中")
    else:
        print("[filter] 过滤后没有任何样本可处理（检查 max_id/skip_ids/rel_map 是否匹配）")
        return
...
if __name__ == "__main__":
    ...
    # 例如：_intermediate_representations/tiq/seq2seq_tqu/train_tqu-faith.json
    ...
    # 只跑 Id<=3000，并跳过 2525
    run_one(
        CONFIG, BASE_TSF, REL_PROMPT1, REL_KEY,
        "out/fer_prompt1_id_le_3000_skip2525.jsonl",
        SOURCES, max_id=MAX_ID, skip_ids=SKIP_IDS
    )

    run_one(
        CONFIG, BASE_TSF, REL_PROMPT2, REL_KEY,
        "out/fer_prompt2_id_le_3000_skip2525.jsonl",
        SOURCES, max_id=MAX_ID, skip_ids=SKIP_IDS
    )
