...
INPUT_PATH = "simplified_dev.json"
OUTPUT_PATH = "324移除句法分析LLM_测试集_包含显性.json"
...
START_INDEX = None # 手动指定起始 index（优先级最高）
...
def call_llm_for_batch_with_new_client(batch: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    client = OpenAI(api_key=API_KEY, base_url=BASE_URL)
    return call_llm_for_batch(client, MODEL, batch)
...
def run_file(client: OpenAI):
    ...
    print(
        ...
        f"batch_size={BATCH_SIZE}, workers={MAX_WORKERS}, saved={len(existing_by_id)}"
    )
    t0 = time.time()

    pending_batches = build_batches(data, start_index, end_index, BATCH_SIZE, existing_by_id)
    if not pending_batches:
        print("没有需要处理的新 batch。")
        return

    for item in pending_batches:
        print(
            f"[queue] idx {item['start']}~{item['end']} | new={len(item['batch'])} | "
            f"ids={item['ids'][:5]}{'...' if len(item['ids']) > 5 else ''}"
        )

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_meta = {
            executor.submit(call_llm_for_batch_with_new_client, item["batch"]): item
            for item in pending_batches
        }
        ...
        for future in as_completed(future_to_meta):
            ...
            try:
                out_items = future.result()
            except Exception as e:
                print(
                    f"[error] idx {meta['start']}~{meta['end']} | ids={meta['ids']} | error={e}"
                )
                continue
            ...
            print(
                f"[saved] idx {meta['start']}~{meta['end']} | got={got}/{len(meta['batch'])} | "
                f"total_saved={len(merged)} | elapsed={elapsed:.1f}s -> {OUTPUT_PATH}"
            )
...
# TARGET_IDS = {840, 1829, 2279, 2789, 2819, 4839}
# def run_file(client: OpenAI):
#     data = load_json_list(INPUT_PATH)
#     if not data:
#         raise RuntimeError(f"输入文件为空或不是 JSON list：{INPUT_PATH}")
